...
exports.disable_region_product = params => {
	...
	try {
		...
		console.log(clc.red('В функции disable_region_product проиошла ошибка:'), clc.white(err.message));
	}
	return result.promise;
};
...
exports.get_product_city = params => {
  let result = q.defer()
  let sql = 'SELECT * FROM geo_product WHERE uid=? AND rid=? AND (cid is NOT null OR cid!="") AND status="true"';
  let param = [params.uid, params.rid];
  try {
    db.query(sql, param, (err, rows) => {
      result.resolve(rows);
...
exports.add_city_product = params => {
	...
	let param = [params.uid, params.data_region.rid, params.data_region.name, params.data_region.kladr, params.data_city.cid, params.data_city.name, params.data_city.city_type, params.data_city.kladr, 'true'];
	try {
		db.query(sql, param, (err, rows) => {
			result.resolve(rows);
		});
...
exports.update_city_product = params => {
	...
	try {
		...
		console.log(
			clc.red('В функции update_city_product проиошла ошибка:'),
			clc.white(err.message),
		);
	}
	return result.promise;
